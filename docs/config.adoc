...
Actions are basically types of functionalities that can be assigned to keys 
to produce specific sequences and/or combination of keypresses that will be sent back to system.
In other words, actions allow remapping your keyboard in a specific way.

There are currently more than 30 different types of actions available in kanata.
Moreover, many actions have multiple variants, which gives even more possibilities.

To make it less overwhelming, all available actions have been split into 4 categories: 
- Essential - Must-know actions, most of them appear in practically all kanata configs.  
- Extra - Still somewhat basic actions, but less commonly used.
- Advanced - Actions that are recommended to read about after getting some grasp of kanata. 
- Mouse - Normally mouse actions would be put in Extra category, but there are so many of them, they deserve their own category.
